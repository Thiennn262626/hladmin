...
import { Form, Input, Upload, Space, Button, Table } from "antd";
import { DeleteOutlined } from "@ant-design/icons";
import { InputNumber } from "antd";
import { useSelector, useDispatch } from "react-redux";
import { setDataAttribute } from "../../Product/counterProduct";

const Index = ({ index }) => {
  const dispatch = useDispatch();
  const dataAttributes = useSelector(
    (state) => state.counterProduct.dataAttribute
  );
  ...
  const items = useSelector((state) => state.counterProduct.items);
  ...
  const handleRemove = (record) => {
    ...
    setData(newData);
    const newDataAttribute = dataAttributes.filter(
      (item) => item.locAttributeValueName !== record.locAttributeValueName
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => <p>{record?.locAttributeValueName}</p>,
    },
    {
      title: "TotalStock",
      dataIndex: "totalStock",

      render: (_, record) => <p>{record?.totalStock}</p>,
    },
    {
      title: "Price",
      dataIndex: "price",
    ...
    },
    {
      ...
      render: (_, record) => <p>{record?.priceBefore}</p>,
    },
    {
      title: "Action",
      render: (_, record) => (
        ...
        <div className="w-[50%]   border rounded-[12px] m-[10px]">
          ...
          <div className="" key={index}>
            ...
            <Form.Item label="Name" className="!w-full">
              <Input
                value={locAttributeValueName}
                maxLength={120}
                onChange={onChangelocAttributeValueName}
              />
            </Form.Item>
