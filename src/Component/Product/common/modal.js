...
const App = () => {
  ...
  const modalSkus = useSelector((state) => state.counterProduct.modalSkus);
  console.log(modalSkus);
  const productID = useSelector((state) => state.counterProduct.productID);
  const handleCancel = () => {
  ...
  useEffect(() => {
    ...
    if (modalSkus && productID !== "") fetchProduct();
    return () => {
      setListSku([]);
    };
  }, [modalSkus, productID]);
  ...
  const columns = [
    ...
    },
    {
      title: "Price",
      dataIndex: "price",
      render: (_, record) => <p> {VND.format(record?.price)}</p>,
    },
    {
      title: "Quantity",
      dataIndex: "quantity",
      render: (_, record) => (
        <div className="flex w-full justify-around">
          ...
          <Dropdown
            ...
            dropdownRender={(menu) => (
              <div style={contentStyle} className="w-[150px]">
      ...
      render: (_, record) => (
        <>
          {record?.attribute.length === 1 ? (
            ...
            <p>
              {" "}
              {`${record?.attribute[0]?.locAttributeValueName} - ${record?.attribute[1]?.locAttributeValueName}`}{" "}
            </p>
          ) : null}
        </>
      ),
